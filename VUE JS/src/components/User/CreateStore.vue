    ...
    <form class="contact-form-style">
      ...
      <div class="row">
        ...
        <div class="col-lg-12">
          ...
          ></textarea>
          <button @click="createStore()" type="button">SEND</button>
...
export default {
  ...
  methods: {
    ...
    createStore() {
      ...
      const url = this.$store.state.Api + "AddStore";
      ...
      axios
        ...
        .then((response) => {
          ...
          this.$store.commit("UserStoreId", response.data.storeId);
    ...
    updateRole(r) {
      this.$store.commit("UpdateUserRole", r);
      const url = this.$store.state.Api + "UpdateUserRole/" + this.user.id;
      axios.post(url, { role: r }).then((response) => {
        //    console.log(response.data);
      });
    },
  },
